...
TEST_F(HashTableTest, SingleSkipListInsertionSpeed) {
    bucket_t bucket;
    // test skiplist
    int collision = 0;
    auto start = env->NowMicros();
    for (int64_t i = 0; i < FLAGS_num; ++i) {
        const int64_t k = (trace->Next() % FLAGS_num);
        char key[100];
        snprintf(key, sizeof(key), "%016" PRIu64 "", k);
        const char* value = bucket.GenerateEntry(i, kTypeValue, key, gen.Generate(FLAGS_value_size));
        bucket.table->Insert((KeyHandle)value);
    }
    auto duration = env->NowMicros() - start;
    fprintf(stdout, "Bucket, %d, Skiplist_Random_Insertion_Speed, %f , Kops/s (num %d each bucket). . === \n", 
                (int) FLAGS_partition, 
