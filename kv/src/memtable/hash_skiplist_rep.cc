...
class HashSkipListRep : public MemTableRep {
 public:
  HashSkipListRep(const MemTableRep::KeyComparator& compare,
                  Arena* allocator, const SliceTransform* transform,
 ...
 private:
  ...
  Arena* const allocator_;

  inline size_t GetHash(const Slice& slice) const {
    return MurmurHash(slice.data(), static_cast<int>(slice.size()), 16) %
           bucket_size_;
  }
...
HashSkipListRep::Bucket* HashSkipListRep::GetInitializedBucket(
  ...
  // if (bucket->Size() > 3) printf("bucket %05d: length: %04d. %s\n", hash, bucket->Size(), transformed.ToString().c_str());
  return bucket;
}
