...
cgexec -g memory:kv64 ./kv/release/tests/db/kv_bench --report_interval=$REPORT_INTERVAL --writes=$WRITE --rwdelay=5 --db=/mnt/nvm/kv1B --logpath=/mnt/ssd --threads=3 --open_files=20000  --range=1000000000 --num=1000000000 --reads=1000000 --bloom_bits=10 --low_pool=2 --high_pool=2  --stats_interval=1000000 --value_size=100  --benchmarks=readexpwhilewriting,stats        --ycsb_ops_num=$YCSB_OPS   --use_existing_db=true --histogram=true --log=false --write_buffer_size=2097152 --skiplistrep=false --sleep=$SLEEP --print_wa=false --log_dio=true  --hugepage=true | tee kv_readexpwhilewriting.log                  
...
echo fb0-=0-= | sudo -S fstrim /mnt/nvm
echo fb0-=0-= | sudo -S bash -c 'echo 1 > /proc/sys/vm/drop_caches'
cgexec -g memory:kv64 ./pebblesdb/release/db_bench   --report_interval=$REPORT_INTERVAL --writes=$WRITE --rwdelay=3 --db=/mnt/nvm/peb1B --logpath=/mnt/ssd --threads=3 --open_files=20000 --range=1000000000 --num=1000000000 --reads=1000000 --bloom_bits=10 --bg_threads=3              --stats_interval=1000000 --value_size=100  --benchmarks=readexpwhilewriting,stats          --ycsb_ops_num=$YCSB_OPS  --use_existing_db=true --histogram=true    --log=false --write_buffer_size=67108864  --sleep=$SLEEP | tee peb_readexpwhilewriting.log
...
echo fb0-=0-= | sudo -S fstrim /mnt/nvm
echo fb0-=0-= | sudo -S bash -c 'echo 1 > /proc/sys/vm/drop_caches'
cgexec -g memory:kv64 ./leveldb/release/db_bench  --report_interval=$REPORT_INTERVAL  --writes=$WRITE --rwdelay=5 --db=/mnt/nvm/level1B --logpath=/mnt/ssd --threads=3 --open_files=20000 --range=1000000000 --num=1000000000 --reads=1000000 --bloom_bits=10                             --stats_interval=1000000 --value_size=100  --benchmarks=readexpwhilewriting,stats           --ycsb_ops_num=$YCSB_OPS  --use_existing_db=1  --histogram=1       --log=false  --max_file_size=67108864 --write_buffer_size=67108864  --sleep=$SLEEP --print_wa=true | tee level_readexpwhilewriting.log
