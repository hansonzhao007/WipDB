...
gen_exponential(struct GenInfo * const gi)
{
  ...
  struct GenInfo *
generator_new_exponential(const double percentile, double range)
{
  range = range * 0.15;
  struct GenInfo * const gi = (typeof(gi))malloc(sizeof(*gi));
...
zeta_range(const uint64_t start, const uint64_t count, const double theta)
{
  double sum = 0.0;
  if (count > 0x10000000) {
    fprintf(stderr, "zeta_range would take a long time... kill me or wait (%llu)\n", (unsigned long long)count);
  }
...
// ===================================================
  static uint64_t
gen_normal(struct GenInfo * const gi)
{
    double p;
    double val = 0;
    double random = 0;
    do {
      ...
      val = (random + 5) * (gi->gen.normal.max - gi->gen.normal.min) / 10;
    }while(val < gi->gen.normal.min || val > gi->gen.normal.max);
...
struct GenInfo *
generator_new_normal(uint64_t min, uint64_t max)
{
  struct GenInfo * const gi = (typeof(gi))malloc(sizeof(*gi));
  gi->gen.normal.min = min;
  gi->gen.normal.max = max;
  gi->gen.normal.mean = (max + min) / 2;
  gi->gen.normal.stddev = (max - min) / 4;
  gi->type = GEN_NORMAL;
  gi->next = gen_normal;
