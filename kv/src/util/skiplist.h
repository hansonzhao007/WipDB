...
#ifndef KV_DB_SKIPLIST_H_
#define KV_DB_SKIPLIST_H_
...
class SkipList {
 ...
 public:
  ...
  explicit SkipList(Comparator cmp, Arena* allocator,
                    int32_t max_height = 12, int32_t branching_factor = 4);

  // Insert key into the list.
  // REQUIRES: nothing that compares equal to key is currently in the list.
  void Insert(const Key& key);
  void InsertToPool(Key key);
  void FlushPool();
 ...
 private:
  ...
  // node pool
  std::vector<Key> pool_;

  // Immutable after construction
  Comparator const compare_;
  Arena* const allocator_;    // Arena used for allocations of nodes
...
template <typename Key, class Comparator>
SkipList<Key, Comparator>::SkipList(const Comparator cmp, Arena* allocator,
                                    int32_t max_height,
...
template<typename Key, class Comparator>
void SkipList<Key, Comparator>::InsertToPool(Key key) {
    pool_.push_back(key);
}


template<typename Key, class Comparator>
void SkipList<Key, Comparator>::FlushPool() {
    for (auto key : pool_) {
        Insert(key);
    }
    pool_.clear();
