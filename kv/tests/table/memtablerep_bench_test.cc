...
int main(int argc, char** argv) {
  ...
  std::unique_ptr<kv::MemTableRepFactory> factory;
  if (FLAGS_memtablerep == "skiplist") {
    factory.reset(new kv::SkipListFactory);
  } else if (FLAGS_memtablerep == "hashskiplist") {
    factory.reset(kv::NewHashSkipListRepFactory(
        FLAGS_bucket_count, FLAGS_hashskiplist_height,
        FLAGS_hashskiplist_branching_factor));
    options.prefix_extractor.reset(
        kv::NewFixedPrefixTransform(FLAGS_prefix_length));
  } else if (FLAGS_memtablerep == "cuckoo") {
    factory.reset(kv::NewHashCuckooRepFactory(
  ...
  kv::InternalKeyComparator internal_key_comp(
      kv::BytewiseComparator());
  kv::MemTable::KeyComparator key_comp(internal_key_comp);
  kv::Arena arena;
  kv::WriteBufferManager wb(FLAGS_write_buffer_size);
  uint64_t sequence;
  ...
  std::unique_ptr<kv::MemTableRep> memtablerep;
  kv::Random64 rng(FLAGS_seed);
  const char* benchmarks = FLAGS_benchmarks.c_str();
  while (benchmarks != nullptr) {
    std::unique_ptr<kv::KeyGenerator> key_gen;
    ...
    if (sep == nullptr) {
      ...
      name = kv::Slice(benchmarks, sep - benchmarks);
      benchmarks = sep + 1;
    }
    std::unique_ptr<kv::Benchmark> benchmark;
    if (name == kv::Slice("fillseq")) {
      memtablerep.reset(createMemtableRep());
      key_gen.reset(new kv::KeyGenerator(&rng, kv::SEQUENTIAL,
                                              FLAGS_num_operations));
      benchmark.reset(new kv::FillBenchmark(memtablerep.get(),
                                                 key_gen.get(), &sequence));
    } else if (name == kv::Slice("fillrandom")) {
      memtablerep.reset(createMemtableRep());
      key_gen.reset(new kv::KeyGenerator(&rng, kv::UNIQUE_RANDOM,
                                              FLAGS_num_operations));
      benchmark.reset(new kv::FillBenchmark(memtablerep.get(),
                                                 key_gen.get(), &sequence));
    } else if (name == kv::Slice("readrandom")) {
      key_gen.reset(new kv::KeyGenerator(&rng, kv::RANDOM,
                                              FLAGS_num_operations));
      benchmark.reset(new kv::ReadBenchmark(memtablerep.get(),
                                                 key_gen.get(), &sequence));
    } else if (name == kv::Slice("readseq")) {
      key_gen.reset(new kv::KeyGenerator(&rng, kv::SEQUENTIAL,
                                              FLAGS_num_operations));
      benchmark.reset(
          new kv::SeqReadBenchmark(memtablerep.get(), &sequence));
    } else if (name == kv::Slice("readwrite")) {
      memtablerep.reset(createMemtableRep());
      key_gen.reset(new kv::KeyGenerator(&rng, kv::RANDOM,
                                              FLAGS_num_operations));
      benchmark.reset(new kv::ReadWriteBenchmark<
          kv::ConcurrentReadBenchmarkThread>(memtablerep.get(),
                                                  key_gen.get(), &sequence));
    } else if (name == kv::Slice("seqreadwrite")) {
      memtablerep.reset(createMemtableRep());
      key_gen.reset(new kv::KeyGenerator(&rng, kv::RANDOM,
                                              FLAGS_num_operations));
      benchmark.reset(new kv::ReadWriteBenchmark<
          kv::SeqConcurrentReadBenchmarkThread>(memtablerep.get(),
                                                     key_gen.get(), &sequence));
    } else {
