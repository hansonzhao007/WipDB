...
static void
test_gen(struct GenInfo * gi, const char * const tag, const uint64_t count)
{
  ...
  printf("usec: %llu MIN: %llu, MAX: %llu\n", (unsigned long long) dt, (unsigned long long) r[0], (unsigned long long) r[count-1]);
...
main(int argc, char ** argv)
{
  ...
  const char * tags[11] = {"CONST", "COUNTER", "EXP", "ZIPF",
  ...
  for (uint64_t i = 0x100000; i < 0x1000000; i<<=1) {
    ...
    printf("gen 0x%llx zipfian .. ", (unsigned long long) i);
