...
Arena::Arena(HugePageBlock* hpblock) : memory_usage_(0) {
  hpblock_ = hpblock;
...
Arena::~Arena() {
  for (size_t i = 0; i < blocks_.size(); i++) {
    if (hpblock_ != nullptr) {
      hpblock_->ReleaseBlock(blocks_[i]);
    }
    else delete[] blocks_[i];
  }
}

char* Arena::AllocateFallback(size_t bytes) {
  // // TODO: consider delete this condition
